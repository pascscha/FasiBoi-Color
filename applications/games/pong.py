...
class Pong(core.Game):
    ...
    def _update_midgame(self, io, delta):
        ...
        if 0 < self.ball_y < 1:
            if self.ai_platform_pos - self.ai_platform_width/2 < self.ball_x < self.ai_platform_pos + self.ai_platform_width/2:
            ...
            if self.platform_pos - self.platform_width/2 - 1 < self.ball_x < self.platform_pos + self.platform_width/2:
                self.ball_direction = - \
        ...
        io.display.update(round(self.ball_x), round(
            self.ball_y), (255, 255, 255))

        for x in range(round(self.ai_platform_pos-self.ai_platform_width/2), round(self.ai_platform_pos + self.ai_platform_width/2)):
            io.display.update(x, 0, (255, 0, 0))

        for x in range(round(self.platform_pos-self.platform_width/2), round(self.platform_pos + self.platform_width/2)):
