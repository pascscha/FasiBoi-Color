...
class ReversiField(alphabeta.BitField):
    def __init__(self, count1=2, count2=2, bits1=0x18000000, bits2=0x1800000000):
    ...
    def _playerscore(self, player):
        ...
        return (
            1000 * self.count_bits(bits & 0x8100000000000081)
    ...
    def possible_moves(self, player):
        ...
        if self.possible[index] is None:
            ...
            possible_bitmap = (
                self.possible_dir(player_bits, opponent_bits, free, self.move_n)
            ...
            self.possible[index] = [
                ReversiMove(x, y, player)
                for x in range(self.width)
                for y in range(self.height)
                if possible_bitmap & self.get_bitmask(x, y) != 0
            ]
    ...
    def flipped_stones(self, x, y, player):
        ...
        for dx, dy in [
            ...
            while (
                ...
                and self.get_value(x2, y2) == other
            ):
                flips_dir |= self.get_bitmask(x2, y2)
                count_dir += 1
                x2 += dx
            ...
            if (
                0 <= x2 < self.width
                and 0 <= y2 < self.height
                and self.get_value(x2, y2) == player
            ):
                count += count_dir
                flips |= flips_dir
        return flips, count
    ...
    def game_over(self):
        return self.is_full() or (
            len(self.possible_moves(self.COLOR1)) == 0
            and len(self.possible_moves(self.COLOR2)) == 0
        )
    ...
    def winning_moves(self):
        if self.has_won(self.COLOR1):
            ...
            return []
        return [
            ReversiMove(x, y, color)
            for x in range(self.width)
            for y in range(self.height)
            if bits & self.get_bitmask(x, y) != 0
        ]
