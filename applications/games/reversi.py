...
class ReversiField(alphabeta.BitField):
    def __init__(
            ...
            bits1=0x18000000,
            bits2=0x1800000000):
        super().__init__(width=8, height=8, bits1=bits1, bits2=bits2)
    ...
    def possible_moves(self, player):
        ...
        if self.possible[index] is None:
            ...
            possible_bitmap = self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_n) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_ne) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_e) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_se) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_s) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_sw) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_w) | self.possible_dir(
                player_bits,
                opponent_bits,
                free,
                self.move_nw)
            ...
            self.possible[index] = [
                ReversiMove(
                    x,
                    y,
                    player) for x in range(
                    self.width) for y in range(
                    self.height) if possible_bitmap & self.get_bitmask(
                    x,
                    y) != 0]
    ...
    def flipped_stones(self, x, y, player):
        ...
        for dx, dy in [(-1, -1), (-1, 0), (-1, 1), (0, -1),
            ...
            x2 += dx
            y2 += dy
            while 0 <= x2 < self.width and 0 <= y2 < self.height and self.get_value(
                    x2, y2) == other:
                flips_dir |= self.get_bitmask(x2, y2)
                count_dir += 1
                x2 += dx
            ...
            if 0 <= x2 < self.width and 0 <= y2 < self.height and self.get_value(
                    x2, y2) == player:
                count += count_dir
                flips |= flips_dir
        return flips, count
    ...
    def game_over(self):
        return self.is_full() or (len(self.possible_moves(self.COLOR1)) ==
                                  0 and len(self.possible_moves(self.COLOR2)) == 0)
    ...
    def winning_moves(self):
        ...
        return [
            ReversiMove(
                x,
                y,
                color) for x in range(
                self.width) for y in range(
                self.height) if bits & self.get_bitmask(
                x,
                y) != 0]
