...
class Racer(core.Game):
    ...
    def load_sprite(self, name, alpha=False):
        if alpha:
            return cv2.cvtColor(
                cv2.imread(
                    os.path.join(self.RESOURCES_PATH, name), cv2.IMREAD_UNCHANGED
                ),
                cv2.COLOR_RGB2BGR,
            )
        else:
            return cv2.cvtColor(
                cv2.imread(os.path.join(self.RESOURCES_PATH, name)), cv2.COLOR_RGB2BGR
            )
    ...
    def reset(self, io):
        ...
        for o in range(4):
            self.obstacles.append(
                (
                    10 + o * 5 * int(self.obstacle_distance),
                    random.choice(self.obstacle_sprites),
                    random.choice([0, 1]),
                )
            )
    ...
    def draw(self, io, delta):
        ...
        while (d - self.obstacles[0][0]) > io.display.height:
            ...
            self.obstacles.append(
                (
                    self.obstacles[-1][0] + 5 * int(self.obstacle_distance),
                    random.choice(self.obstacle_sprites),
                    random.choice([0, 1]),
                )
            )
        ...
        for (distance, sprite, lane) in self.obstacles:
            bitmaputils.apply_image(
                sprite, io.display, (lane_x - 6 + lane * 8, int(d - distance))
            )

        bitmaputils.apply_image(
            self.car_sprite,
            io.display,
            (int(l * (io.display.width - 5)), io.display.height - 5),
        )

        if self.fuel < 0.5:
            fuel_color = animations.blend_colors(RED, YELLOW * 0.5, self.fuel * 2)
        else:
            fuel_color = animations.blend_colors(
                YELLOW * 0.5, GREEN * 0.5, (self.fuel - 0.5) * 2
            )
